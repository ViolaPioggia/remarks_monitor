...
func (l *MapWorkLogic) MapWork(in *map1.GetMapWorkReq) (*map1.GetMapWorkResp, error) {
	MNum := in.MNum
	num, _ := strconv.ParseInt(MNum, 10, 64)
	File := in.Paths
	DoMapTask(File, num)

	return &map1.GetMapWorkResp{Director: tool.GetWD() + "/remarks_monitor/data/remarks_monitor/MapReduce"}, nil
}
